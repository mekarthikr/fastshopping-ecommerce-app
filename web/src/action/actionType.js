export const GET_USERS = "GET_USERS";
export const DELETE_USER = "DELETE_USER";
export const ADD_USER = "ADD_USER";
export const UPDATE_USER = "UPDATE_USER";
export const GET_SINGLE_USER = "GET_SINGLE_USER";

export const GET_PRODUCTS = "GET_PRODUCTS";
export const DELETE_PRODUCT = "DELETE_PRODUCT";
export const GET_SINGLE_PRODUCT = "GET_SINGLE_PRODUCT";
export const UPDATE_PRODUCT = "UPDATE_PRODUCT";
export const ADD_PRODUCT = "ADD_PRODUCT";

export const ADD_TO_CART = "ADD_TO_CART";
export const STATE = "STATE";
export const REMOVE_FROM_CART = "REMOVE_FROM_CART";

export const USER_LOGGED_IN = "USER_LOGGED_IN";
export const USER_LOGGED_OUT = "USER_LOGGED_OUT";
export const USER_LOGING_IN = "USER_LOGING_IN";
export const USER_LOGGED_IN_SUCCESS = "USER_LOGGED_IN_SUCCESS";
export const USER_LOGGED_IN_FAILED = "USER_LOGGED_IN_FAILED";
export const CLEAR_PRODUCT = "CLEAR_PRODUCT";
export const INSERT_TO_CART = "INSERT_TO_CART";
export const USER_IS_LOGGED_IN = "USER_IS_LOGGED_IN";

export const ADMIN_LOGGED_IN_SUCCESS = "ADMIN_LOGGED_IN_SUCCESS";
export const ADMIN_LOGGED_IN_FAILED = "ADMIN_LOGGED_IN_FAILED";
export const SET_CART = "SET_CART";
export const ADD_USER_TOKEN = "ADD_USER_TOKEN";
export const USER_IS_LOGGED_OUT = "USER_IS_LOGGED_OUT";
export const ADD_ADMIN_TOKEN = "ADD_ADMIN_TOKEN";
export const ADMIN_IS_LOGGED_OUT = "ADMIN_IS_LOGGED_OUT";
export const RETAIN_USER_DETAILS = "RETAIN_USER_DETAILS";

export const USER_ADDED_SUCCESS="USER_ADDED_SUCCESS";
export const USER_ADDED_FAILED="USER_ADDED_FAILED";
export const RESET_REGISTER="RESET_REGISTER";

export const ACTION_RESPONSE_MESSAGE="ACTION_RESPONSE_MESSAGE";
export const INCREASE_PAGE_COUNT="INCREASE_PAGE_COUNT";
export const DECREMENT_PAGE_COUNT="DECREMENT_PAGE_COUNT";



